...
class VisualizerCliTests(unittest.TestCase):
    ...
    def run_visualizer(self, command: list[str], output_name: str) -> str:
        ...
        self.assertIn("pinnedMarkers: []", html)
        self.assertIn('addEventListener("click", event => handleCanvasClick(event, key))', html)
        self.assertIn("function togglePinnedMarker(event, key)", html)
        self.assertIn("function renderPinnedMarkers(key)", html)
        self.assertIn("function pinnedMarkerSummary(key)", html)
        self.assertIn("PIN_MARKER_COLORS", html)
        self.assertIn("item.id === marker.id", html)
